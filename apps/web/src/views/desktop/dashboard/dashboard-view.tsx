...
import { AutomationsBrowser } from '@/views/desktop/automations-browser';
import {
  getAutomations,
...
import { MetaMark, StatusPill, ZernioMark } from '@/views/shared/platform-badge';
import {
  AutomationsTableSkeleton,
  CardSkeleton,
  StatCardsSkeleton,
} from '@/views/desktop/skeleton';
import { SyncButton } from '@/views/desktop/sync-button';

// Status messages are handled globally by ToastHost (views/shared/toast.tsx), which reads the same
// ?instagram= / ?automation= params the server actions redirect with.

export async function DesktopDashboardView() {
  ...
  let organization;
  try {
    // The organization the switcher has selected (an `org` cookie, validated against the
    // caller's memberships), else their first one - see lib/organization.ts.
    organization = (await getActiveOrganization()).active;
  } catch (error) {
    return (
